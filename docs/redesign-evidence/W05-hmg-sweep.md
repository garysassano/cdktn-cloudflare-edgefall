...
The HMG now moves through authored intermediate barrel elevations while retaining the existing fire/action ownership. Both combat inspectors show the accepted hand-to-muzzle segment; matching integer velocities drive the actual swept bullets. Portable replay, SQLite recovery and the local keyboard inspector pass. The automatic four-browser gate has not passed: three attempts reproduced existing input-lead and host-clock failures. The [acceptance audit](W05-acceptance-audit.md) also requires an integrated destructible-support combat fixture, and W06 must review final frames, sound and feel before remaining final media production.
