...
Room mode is indexed from `lobby, loading, playing, intermission, paused-empty, recovering, completed, expired`. The minimum frame is 444 bytes and the base sections can reach 39,416 bytes; the maximum combat section raises the complete frame limit to 52,044 bytes. This hard allocation limit is not the 6 KiB steady-traffic target; W02/W04 must measure representative populated rooms and reduce traffic before accepting the performance gate. No compression or render quantization is implied by this format.
...
| Controlled players        | 300          | Player count     |
...
Header flag bit 0 appends a versioned combat section after removal IDs; flags other than 0 or 1 are rejected. Flag 0 decodes to `combat: null`, used by controller and synthetic fixtures. Combat snapshots include the section, including when the encounter has completed. All frames and handshakes now require protocol 3.9; there is no compatibility decoder for earlier minors.
...
The 30-tick bash projects an attached threat with attack 6 and shape 12 from windup through its four active ticks (offsets 12–15). It uses the enemy body as source, a global action identity, committed target and facing, and an authored hand socket. This projection and the shield-break event extend protocol 3.5 without increasing snapshot or event record sizes. See the [private continuation contract](combat-checkpoint-v10.md).
