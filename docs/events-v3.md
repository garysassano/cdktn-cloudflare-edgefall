...
All fields are little endian. A type-3 event batch contains a 32-byte header and one to 64 fixed 96-byte records, at most 6,176 bytes. The run/connection identity belongs to the enclosing batch. Decoders validate its identity, exact size, content IDs, enum values, reserved fields, complete cursor sequence and event identity ordering before publishing any event.
...
| 24 / 26       | Record count / record byte size `96`            | 2 × u16 |
| 28            | Reserved zero                                   | u32     |

| Record offset          | Field                                                                                                                                                                            | Type    |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| 0 / 4 / 8              | Delivery cursor / tick / within-tick counter                                                                                                                                     | 3 × u32 |
| 12                     | Kind: shot `0`, sound `1`, muzzle-blocked `2`, impact `3`, killed `4`, melee `5`, throw `6`, action-sound `7`, explosion `8`, shield-break `9`, prop-destroyed `10`, pickup `11` | u32     |
...
| 76 / 80 / 84 / 88 / 92 | Claim ID / new weapon / previous weapon / previous ammunition / resulting ammunition; all zero for non-pickup events                                                             | 5 × u32 |
...
## Confirmed individual pickups

Protocol 3.13 appends pickup kind 11 and five grant words, bringing every event record to 96 bytes. A pickup uses player origin, the receiving player as owner, action ID and marker index zero, source ID as the content definition, item ID as target, and the authored sensor origin as x/y. Material is none; firearm confirmation and beam fields are absent. It does not allocate a firearm action or spend a shot ordinal. The registered item definition binds the source, unique claim ID, weapon, capacity and active lifetime.

The two weapon words use one-based indexes in `sidearm, heavy-machine-gun, shotgun, rocket-launcher, flamethrower, laser`; zero means no pickup payload. A grant requires a complete payload. An identical weapon adds exactly the bounded amount and must increase inventory; a different weapon receives its defined ammunition. Sidearm previous ammunition is zero. Unknown references, inconsistent amounts, fields on another event kind, partial payloads and out-of-lifetime claims reject. Availability and claimant state arrive in full snapshots; processing a claim event only presents the already committed result.
...
Protocol 3.15 preserves the event record layout. A tank cannon release carries definition 19 and the same run/player/control/shot/marker confirmation key as other firearms. Both tank hardpoints allocate from their shared ordinal space, so simultaneous releases remain distinct. Contact emits one explosion event and at most one blast damage grant per target; it does not add a direct-hit grant. The released shell retains its original owner and action through exit, disconnect and destruction.
