...
Protocol 3.16 adds thirty-two bytes per vehicle for the dedicated sacrifice phase, original owner/action identity, arming/commit/end ticks and charge direction. [Combat archive 19](combat-checkpoint-v19.md) and simulation/recording format 16 preserve that state. Breakwater and material recordings advance to 6 and 3 because commands now require `specialPressed`. Input, acknowledgment, combat section 5 and 96-byte gameplay event record sizes remain unchanged; older diagnostic versions reject explicitly.
...
Protocol 3.14 raises the complete snapshot allocation ceiling to 53,088 bytes. The loaded scenario ID survives prop destruction, removes geometry inference from surviving entities, and rejects a changed or unknown calibration case. See the current [snapshot](snapshot-v3.md) and [archive](combat-checkpoint-v19.md) contracts.
...
Protocol 3.16 raises vehicle records to 396 bytes and the complete snapshot ceiling to 54,240 bytes. `VehicleSpecial` requires its existing dedicated edge and held bit for thirty consecutive accepted occupied ticks. Release, disconnect, exit or damage cancels arming. Commitment requires safe ejection, advances the player input generation once, consumes the hull and starts an unoccupied charge on the following tick. The original owner/action identity remains authoritative through recovery and the single terminal blast.
