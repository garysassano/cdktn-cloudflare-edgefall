...
export const MAX_COMBAT_BYTES = 8784;
...
function length(members: number, objectives: number, kills: number) {
  ...
  return COMBAT_HEADER_BYTES + members * COMBAT_MEMBER_BYTES + (objectives + kills) * 8;
}
export function combatRecordBytes(combat: CombatSnapshot | null) {
  return combat === null
    ? 0
    : length(combat.members.length, combat.objectives.length, combat.kills.length);
}
...
export function readCombat(r: Reader): CombatSnapshot {
  ...
  check(r.u16() === 1 && r.u16() === COMBAT_HEADER_BYTES, "section version/header");
  const cursors = {
  ...
  const members = r.u16(),
    ...
    kills = r.u16();
  r.zero(2);
  check(start + length(members, objectives, kills) === r.bytes.byteLength, "section length");
  const id = r.u32() || null,
    tick = readTick(r),
