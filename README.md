...
`pnpm test:mission:harbor:route` checks the authored Harbor terrain and shared tank depot in Node, Chromium and local workerd. Solo, two-player and four-player inputs traverse the whole route, backtrack on foot, and continue after every tank is spent. Explicit stage budgets support traversal beyond the diagnostic one-minute limit. See the [Harbor route evidence](./docs/redesign-evidence/W09-harbor-route.md); mission encounters, camera progression, the siege engine and presentation remain in progress.
