...
`pnpm test:network:controller --combat-guard` runs a fresh mixed shield/rifle encounter with four keyboard clients. Shield infantry brace, advance, commit a turn, telegraph a finite bash and retain separate break/stun state. Jumping, exposed flanks and explosive shield damage provide counterplay while ordinary body hits remain lethal. Protocol 3.6 and archive 7 preserve presentation and private continuation; local combat recordings use format 4. See the [shield evidence](./docs/redesign-evidence/W05-shield-infantry.md). W05 remains open for the lab tank and remaining grenade platform policies.
