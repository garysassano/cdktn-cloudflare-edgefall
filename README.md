...
`pnpm test:network:controller --combat-guard` runs a fresh mixed shield/rifle encounter with four keyboard clients. Shield infantry brace, advance, commit a turn, telegraph a finite bash and retain separate break/stun state. Jumping, exposed flanks and explosive shield damage provide counterplay while ordinary body hits remain lethal. Protocol 3.8 and archive 9 preserve presentation and private continuation; local combat recordings use format 6. See the [shield evidence](./docs/redesign-evidence/W05-shield-infantry.md). The [W05 audit](./docs/redesign-evidence/W05-acceptance-audit.md) identifies the remaining behavior and acceptance work.
...
The `hmg` inspector scenario adds nine authored infantry HMG directions. The barrel advances one heading every two ticks while fire retains its five-tick cadence; each heading uses matching sockets and integer projectile velocity. Portable replay, SQLite recovery and the keyboard inspector pass; the `pnpm test:network:controller --combat-hmg` four-browser gate remains open after input-lead and host-clock failures. See the [HMG evidence](./docs/redesign-evidence/W05-hmg-sweep.md). Protocol 3.8 carries independent barrel continuation in 300-byte player records; archive 9 and recording format 6 reject the prior experimental layouts. Final drawings, sound and feel still need the W06 benchmark review.
