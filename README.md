...
The current Breakwater benchmark uses [individual weapon supplies](./docs/redesign-evidence/W07-weapon-pickups.md), collected on contact with one winner per item. Shared piles show the remaining count; a player must leave and re-enter before taking another item from the same pile. Weapon swaps preserve cooldown and shot identity. Solo, two-player and four-player browser runs finish in 3,036, 3,044 and 2,992 ticks with exact recording replay. Use `pnpm test:mission:breakwater --no-video` for engineering verification. Mission recordings are now format 3; the earlier W06 review download remains pinned to its original capture, and human review is still pending.

The [pickup room integration](./docs/redesign-evidence/W07-pickup-integration.md) carries individual availability and confirmed grants through protocol 3.13, archive 15 and combat recordings 12. `pnpm test:network:controller --combat-pickups` checks four keyboard clients sharing all five weapon families, including dropped and duplicate claim delivery. The `pickups` inspector scenario exposes remaining counts, while the storage harness checks exact inventory and contact latches across rollback and fresh Worker processes.
