...
describe("profile-bound room reservations", () => {
  it("expires only an empty party at its final reservation deadline without extending it on wake", () => {
    let state = admitMember(createMembership(), id, 0, "lobby", 1000);
    state = admitMember(state, other, 1, "lobby", 2000);
    ...
    const admitted = admitMember(state, id, 0, "playing", 92_999);
  ...
  it("rejects foreign slot claims and one identity controlling multiple slots", () => {
    const joined = admitMember(createMembership(), id, 0, "lobby", 1000);
    expect(() => admitMember(joined, other, 0, "lobby", 1001)).toThrow("slot-reserved");
    expect(() => admitMember(joined, other, 1, "playing", 1001)).toThrow("in-progress");
    expect(() => admitMember(joined, id, 1, "playing", 1001)).toThrow("profile-slot-mismatch");
  });
  it("fences old closes on connected takeover and preserves join order on reentry without reclaiming host", () => {
    let state = admitMember(createMembership(), id, 0, "lobby", 1000);
    state = admitMember(state, other, 1, "lobby", 1001);
    const replaced = admitMember(state, id, 0, "playing", 1002);
    ...
    const returned = admitMember(state, id, 0, "playing", 1005);
  ...
  it("uses a strict 90-second reservation and never admits strangers mid-run after expiry", () => {
    const state = disconnectMember(
      admitMember(createMembership(), id, 0, "lobby", 1000),
      0,
      1,
      2000,
    );
    expect(admitMember(state, id, 0, "playing", 91_999).members[0]?.connected).toBe(true);
    expect(() => admitMember(state, id, 0, "playing", 92_000)).toThrow("reconnect-window-expired");
    expect(() => admitMember(state, other, 0, "playing", 92_000)).toThrow("in-progress");
    expect(admitMember(state, other, 0, "lobby", 92_000).members[0]?.profileId).toBe(other);
  });
  it("persists the heartbeat and does not extend the reconnect window through repeated cold restarts", () => {
    const state = checkpointMembership(admitMember(createMembership(), id, 0, "lobby", 1000), 5000);
    ...
    expect(admitMember(twice, id, 0, "playing", 94_999).members[0]?.generation).toBe(2);
    expect(() => admitMember(twice, id, 0, "playing", 95_000)).toThrow("reconnect-window-expired");
  });
  it("rejects corrupt durable ownership and counter exhaustion", () => {
    const state = admitMember(createMembership(), id, 0, "lobby", 1000);
    ...
    expect(() => admitMember(state, id, 0, "playing", 1001)).toThrow(/generation/);
  });
});
