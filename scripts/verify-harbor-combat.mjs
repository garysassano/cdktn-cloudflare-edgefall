...
import { harborInfantryProof } from "../test/fixtures/harbor-combat-proof.js";
...
const output = "dist/harbor-combat-proof";
...
const browserSource = await bundle("browser", {
  entryPoints: ["test/fixtures/harbor-combat-proof.ts"],
  format: "iife",
  globalName: "EdgefallHarborCombat",
  platform: "browser",
  target: "es2022",
});
const workerSource = await bundle("worker", {
  stdin: {
    ...
    contents: `import { harborInfantryProof } from './test/fixtures/harbor-combat-proof.ts';
export default { fetch(request) {
  return new URL(request.url).pathname === '/proof'
    ? Response.json(harborInfantryProof()) : Response.json({ status: 'ok' });
} };`,
  },
  platform: "neutral",
  format: "esm",
...
let phase = "Node";
try {
  const expected = harborInfantryProof();
  phase = "workerd";
  ...
  assert.deepEqual(
    await page.evaluate("EdgefallHarborCombat.harborInfantryProof()"),
    expected,
  ...
  const report = {
    ...
    scope:
      "Continuous Harbor entrance-to-HMG combat in Node, Chromium and local workerd, with sidearms, dock cover, rifle infantry, a committed hostile grenade, rooftop counterplay, complete input-recording replay and JSON continuation. No full Harbor mission, camera/checkpoint progression, remaining enemy families, boss, SQLite, socket impairment, final media or deployed timing claim.",
  };
